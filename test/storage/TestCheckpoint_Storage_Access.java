...
/** Tests storage server file access methods.
    ...
    then obtains a info for the storage server and checks several properties of
    ...
    locally, and by reading it through the storage server info.
 */
public class TestCheckpoint_Storage_Access extends StorageTest {
    /** Test notice. */
    public static final String notice =
        "checking storage server file access methods (size, read, write)";
    /** Prerequisites. */
    public static final Class[] prerequisites = new Class[] {TestCheckpoint_Storage_Registration.class};
    ...
    /** Creates the <code>TestCheckpoint_Storage_Access</code> object. */
    public TestCheckpoint_Storage_Access() {
    ...
    private void testReadWrite() throws TestFailed {
        ...
        try {
            ...
            response = getResponse("/storage_write", 7001, writeRequest);
        ...
        try {
            ...
            throw new TestFailed("unable to open written file directly for reading", t);
        }
        ...
        // Check the file's size and contents through the storage server info.
        try {
            PathRequest pathRequest = new PathRequest(read_write_file.toString());
            response = getResponse("/storage_size", 7001, pathRequest);
            long size = gson.fromJson(response.body(), SizeReturn.class).size;
            if(size != write_data.length) {
                throw new TestFailed("info reports incorrect file size after " +
        ...
        try {
            ...
            response = getResponse("/storage_read", 7001, readRequest);
            ...
            if(!Arrays.equals(remote_data, write_data)) {
                throw new TestFailed("data retrieved through info from " +
    ...
    private void testAppend() throws TestFailed {
        ...
        try {
            ...
            throw new TestFailed("unable to retrieve file size after appending", t);
        }
