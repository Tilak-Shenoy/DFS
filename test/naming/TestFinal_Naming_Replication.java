 ...
 */
public class TestFinal_Naming_Replication extends NamingTest {
    /** Test notice. */
    public static final String notice = "checking naming server replication policy";
    /** Prerequisites. */
    ...
    /** Info for the storage server initially hosting the file. The other server
        ensures that the naming server commands the copy to be done from this
        server. */
    private ServerInfo hosting_info;
    ...
    protected void perform() throws TestFailed {
        ...
        for(access_counter = 0; access_counter < ACCESS_COUNT; ++access_counter) {
            try {
                ...
                throw new TestFailed("unable to lock " + replicate_file + " for reading", t);
            }
        ...
        try {
            ...
            throw new TestFailed("unable to lock " + replicate_file + " for writing", t);
        }
    ...
    protected void initialize() throws TestFailed {
        ...
        try {
            ...
            hosting_info = hosting_server.start(registration_port,
        ...
        public void copy() {
            this.command_service.createContext("/storage_copy", (exchange -> {
                ...
                if(request.server_ip == null) {
                    failure(new TestFailed("server info argument to copy method null"));
                    sendBooleanReturn(exchange, false, 400);
                    return;
                }
                ...
                synchronized(TestFinal_Naming_Replication.this) {
                    if(replicated) {
                    ...
                    replicated = true;
                    TestFinal_Naming_Replication.this.notifyAll();
                }
    ...
    private class InvalidationStorageServer extends TestStorageServer {
        ...
        InvalidationStorageServer() throws IOException {
            super(TestFinal_Naming_Replication.this);
        }
            ...
            this.command_service.createContext("/storage_delete", (exchange -> {
                ...
                if(!path.equals(replicate_file)) {
                    ...
                    failure(new TestFailed("naming server requested deletion of " +
                            path + ", but deletion of " + replicate_file + " is expected"));
                    return;
                }
