...
public class TestStorageServer {
    ...
    protected HttpServer client_service;
    /** Command interface service. */
    protected HttpServer command_service;
    /** Indicates that the services have been started. This is used to prevent
        multiple starts of the services by alternative implementations of the
        <code>start</code> method. */
    private boolean services_started;
    ...
    public TestStorageServer(Test test) throws IOException {
        this.test = test;
        this.client_service = HttpServer.create(new InetSocketAddress(0), 0);
        this.client_service.setExecutor(Executors.newCachedThreadPool());

        this.command_service = HttpServer.create(new InetSocketAddress(0), 0);
        this.command_service.setExecutor(Executors.newCachedThreadPool());
        services_started = false;
        gson = new Gson();
    }

    /** Starts services for the client and command interfaces.
        ...
        method to prevent repeated calls to <code>start</code> in the services.

     */
    protected synchronized void startServices() {
        // Prevent repeated starting of the services and re-creation of info.
        if(services_started)
            return;

        // Start the client interface service and create the info.
        client_service.start();

        // Start the registration service and create the info.
        command_service.start();

        services_started = true;

        // Register all API to two services
        this.add_client_api();
    ...
    /** Starts the test storage server.
        ...
        @param expect_files An optional list of files which the storage server
                            ...
                            server replies with.
        @return The storage server client interface info.
    ...
    public synchronized ServerInfo start(int naming_register_port, Path[] offer_files,
        ...
        return new ServerInfo(STORAGE_IP, client_service.getAddress().getPort());
    }

    /** Stops the storage server. */
    public void stop() {
        command_service.stop(0);
        client_service.stop(0);
    }

    /** Add APIs supported by client service. */
    ...
    /** Add APIs supported by command service. */
    ...
    public void size() {
        this.client_service.createContext("/storage_size", (exchange -> {
    ...
    /** Throws <code>UnsupportedOperationException</code>. */
    public void read() {
        this.client_service.createContext("/storage_read", (exchange -> {
            sendBooleanReturn(exchange, false, 400);
    ...
    /** Throws <code>UnsupportedOperationException</code>. */
    public void write() {
        this.client_service.createContext("/storage_write", (exchange -> {
            sendBooleanReturn(exchange, false, 400);
    ...
    /** Throws <code>UnsupportedOperationException</code>. */
    public void delete() {
        this.command_service.createContext("/storage_delete", (exchange -> {
            sendBooleanReturn(exchange, false, 400);
    ...
    /** Throws <code>UnsupportedOperationException</code>. */
    public void copy() {
        this.command_service.createContext("/storage_copy", (exchange -> {
            sendBooleanReturn(exchange, false, 400);
