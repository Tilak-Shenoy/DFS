 ...
 */
public class TestCheckpoint_Naming_Registration extends NamingTest {
    /** Test notice. */
    public static final String  notice =
        "checking naming server registration interface";
    /** Prerequisites. */
    public static final Class[] prerequisites = new Class[] {TestCheckpoint_Naming_Path.class};
    ...
    /** Creates the <code>TestCheckpoint_Naming_Registration</code> object. */
    public TestCheckpoint_Naming_Registration() throws IOException {
    ...
    private void testMerging() throws TestFailed {
        ...
        Path[] server1_files = new Path[] {new Path("/file"),
        ...
        Path[] server2_files = new Path[] {new Path("/file"),
                        new Path("/directory/file"),
                        new Path("/another_directory/another_file")};
        Path[] server2_delete_files = new Path[] {new Path("/file"),
                        new Path("/directory/file")};
        ...
        // file to be deleted.
        Path[] server3_files = new Path[] {new Path("/directory"),
        ...
        try {
            synchronized(this) {
                server4.start(registration_port, server4_files, server4_delete_files);
            }
        } catch(Throwable t) {
    ...
    private class UnsuccessfulStorageServer extends TestStorageServer {
        ...
        UnsuccessfulStorageServer() throws IOException {
            super(TestCheckpoint_Naming_Registration.this);
        }
        ...
        synchronized void testDuplicateRegistration() throws TestFailed {
            // Start the storage server services, if they have not already been
            // started.
            try {
                startServices();
            } catch(Throwable t) {
                throw new TestFailed("unable to start services for " +
